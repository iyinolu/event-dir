...
export const AppSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchEvents.fulfilled, (state, { payload }) => {
      ...
      const date = payload.dateQuery;
      const hasCachedRecord = state.eventCache.hasOwnProperty(date);
      if (hasCachedRecord && JSON.stringify(state.eventCache[date]) !== JSON.stringify(payload.data)) {
    ...
    builder.addCase(createNewEvent.fulfilled, (state, { payload }) => {
      ...
      if (hasCachedRecord) {
        state.eventCache = {
          ...state.eventCache,
          [newEventDate]: [...state.eventCache[newEventDate], payload],
        };
      } else {
        state.eventCache = { ...state.eventCache, [newEventDate]: [payload] };
      }
      if (state.event.length && state.event[0]?.event_date === payload.event_date) {
